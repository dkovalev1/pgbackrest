...
testRun(void)
{
    ...
    if (testBegin("verifySetBackupCheckArchive(), verifyLogInvalidResult(), verifyRender()"))
    {
        ...
        KeyValue *testKv = kvNew();

        TEST_RESULT_STR_Z(
            verifyRender(archiveIdResultList, backupResultList, cfgOptionBool(cfgOptVerbose), NULL),
            "\n"
            "  archiveId: 9.6-1, total WAL checked: 1, total valid WAL: 0", "archive: no invalid file list");

        TEST_RESULT_STR_Z(
            verifyRender(archiveIdResultList, backupResultList, cfgOptionBool(cfgOptVerbose), testKv),
            "{\"archives\":["
            "{"
            "\"archiveId\":\"9.6-1\","
            "\"checked\":1,"
    ...
    if (testBegin("cmdVerify() - info files JSON output")){
        ...
        TEST_RESULT_LOG(
            ...
            "P00 DETAIL: unable to open missing file '" TEST_PATH "/repo/archive/db/archive.info' for read\n"
            "P00 DETAIL: unable to open missing file '" TEST_PATH "/repo/archive/db/archive.info.copy' for read\n"
            "P00   INFO: {\"errors\":[\"No usable backup.info file\",\"No usable archive.info file\"],\"stanza\":\"db\",\"status\":\"error\"}"
        );
        ...
        TEST_RESULT_LOG(
            ...
            "P00 DETAIL: invalid checksum, actual 'e056f784a995841fd4e2802b809299b8db6803a2' but expected 'BOGUS'"
            " <REPO:ARCHIVE>/archive.info.copy\n"
            "P00   INFO: {\"errors\":[\"No usable archive.info file\"],\"stanza\":\"db\",\"status\":\"error\"}"
        );
